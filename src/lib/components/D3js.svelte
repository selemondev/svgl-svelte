<script lang="ts">
	...
	const svgContent = () => {
		const restAttrs = Object.entries(rest)
			.map(([k, v]) => `${k}="${v}"`)
			.join(' ');
		return `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="-10 -10 116 111" xmlns:xlink="http://www.w3.org/1999/xlink" width="${width}" height="${height}" ${restAttrs}>
  <clipPath id="clip">
