<script lang="ts">
	...
	const svgContent = () => {
		const restAttrs = Object.entries(rest)
			.map(([k, v]) => `${k}="${v}"`)
			.join(' ');
		return `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 122.88 95.18" style="enable-background:new 0 0 122.88 95.18" xml:space="preserve" width="${width}" height="${height}" ${restAttrs}><style type="text/css"><![CDATA[
	.st0{fill:#0070D1;}
