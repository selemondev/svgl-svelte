...
import { getSvgContent } from '../src/lib/utils';
import { componentTemplate } from '../src/lib/utils';
...
const copySvgComponent = async (svgInfo: Svgl, framework: string) => {
	try {
		...
		if (code) {
			createFiles(
				targetDir,
				`${sanitize(`${svgInfo.title?.split(' ').join('').charAt(0)?.toLocaleUpperCase() + svgInfo.title?.slice(1)}.svelte`)}.svelte`,
				code
			);
		}
	} catch (err) {
...
const exportFiles = async () => {
	try {
		...
		await fs.writeFile(path.join(getRootPath, 'src', 'lib', 'index.ts'), finalOutput, 'utf-8');
	} catch (error) {
